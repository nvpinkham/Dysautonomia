...
#' Use Random forest analysis compare two groups
#'
#'
#' @param infile mat = matrix, map is a dataframe that must have a description column
#' @return mean decrease in accuracy when each feature in the matrix is excluded from the RF
...
#' Remove rare features from matrix
#'
#'
#' @return matrix with feature below the cutoff removed
...
venn2otu <- function(otu, map, 
  ...
  g1.pa <- colSums(otu.g1 > 0)
  g2.pa <- colSums(otu.g2 > 0)
  ...
  grid::grid.newpage()
  VennDiagram::draw.pairwise.venn(area1=length(unique2g1[unique2g1]), 
