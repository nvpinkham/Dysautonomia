...
setwd("/Users/nickpinkham/Desktop/Nature2022_submission/")

map <- read.csv("data/mice/CoHoused_mice.8.16.csv")
...
otu <- read.csv("data/mice/FD_OTU_mice21.csv", 
...
metas <- list.files("data/mice/2020 Mouse Metabolites norm by weight", pattern = "ALL", full.names = T)
...
pair.meta <- function(map.meta, meta, comps, plot = T, simple.return = F){
  ...
  for(i in 1 : length(tp)){
    ...
    res1<- c(tp[i], 
             round(res1 $ aov.tab$`Pr(>F)`[1], 4),
             round(res1 $ aov.tab$F[1], 3), 
             nrow(map.pick))
    
    names(res1) <- c("day", "p.val", "f.stat", "n")  
...
pair.meta2 <- function(map.meta, meta, comps){
  ...
  for(i in 1 : length(tp)){
    ...
    set.seed(42)
    res1 <- vegan::adonis(dist(meta.pick) ~ map.pick$discription, permutations = 9999)
    
    res1 <- res1$aov.tab
    
    ord <- labdsv::pco(dist(meta.pick))
    ...
    plot(ord$points,
         ...
         ylab = paste0("PCO 2 (",
                       round(ord$eig[2] / sum(ord$eig) * 100, 2),
                       "%)"))
    vegan::ordispider(ord, map.pick$discription, col = unique(map.pick$col))
    ...
    if(   rf.res[1] != "VARIABLE p val < 0.05" & res1[1] < 0.05){
      ...
      res2 <- vegan::adonis(dist(meta.pick2) ~ map.pick$discription, permutations = 9999)
      res2 <- res2$aov.tab
...
pair.otu2 <- function(map.otu, otu, comps){
  ...
  for(i in 1 : length(tp)){
    ...
    names(res.i) <- c("DPW", "p.val", "f.stat", "n")  
    ...
    res.i <- c(group.1, group.2, res.i,   g1n,   g2n)
    names(res.i)[7:8] <- c("n group 1", "n group 2")  
...
setwd("/Users/nickpinkham/Desktop/Nature2022_submission/Figure_3/")
