...
source("human_functions.22.05.09.R")
...
alpha.t <- as.data.frame(matrix(nrow = 999, ncol = 4))

rich.t <- beta.t <- alpha.t

colnames(alpha.t) <- c("P_val", "df", "effect_size", "t.stat")
colnames(beta.t) <- c("P_val", "df", "effect_size", "t.stat")
colnames(rich.t) <- c("P_val", "df", "effect_size", "t.stat")
...
postscript(paste0("Figure_1_panel_B_", 
                   Sys.Date(), 
                  ".eps"),
           width = 6, height = 6) 
