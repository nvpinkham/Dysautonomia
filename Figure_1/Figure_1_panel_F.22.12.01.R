...
comp.order <- order(serum.plsda$prop_expl_var$X, decreasing = T)# order component labels by magnitude
sigs <- paste0("component ", comp.order, ": ", sigs, "%")
...
write.csv(source_data.1f,"source_data/source_data_1f.csv", row.names = F)
res <- capture.output(res)
writeLines(res, "Statistical_summaries/Fig_1f_tests.txt")
