...
shared.t <- as.data.frame(matrix(nrow = num.perm, ncol = 6))
...
write.csv(source_data.1d,"source_data/source_data_1d.csv")

shared.t.means <- apply(shared.t, 2, mean)
names(shared.t.means) <- paste("mean over", num.perm, "permutations", names(shared.t.means))
shared.t.means <- capture.output(shared.t.means)
writeLines(shared.t.means, "Statistical_summaries/Fig_1d_tests.txt")
