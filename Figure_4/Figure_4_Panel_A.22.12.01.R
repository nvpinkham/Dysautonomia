...
# no resampling individuals within age bin
...
for(i in 1 : length(treats)){
  for(j in 1 : i){
    if(i != j){
      ...
      res.i <- pair.otu2(map, otu, comps = c(treats[i], treats[j]))
      res <- rbind(res, res.i)
    }
  }
}
...
res$DPW.bin <- res$DPW + (as.numeric(res$dis) * 10) - 15# offset to make more visually pleasing
...
for(i in 1 : 2){
        ...
        points(  res.i $p.val ~ res.i$DPW.bin, bg = res.i$col[1], pch = 21,  cex = res.i$n.scale)
        points(  res.i $p.val ~ res.i$DPW.bin, col = res.i$col[1], type = "l")
}
...
res <- res[,c(1:2,14,3:13)]

write.table(res,"Statistical_summaries/Fig_4a_tests.txt", row.names = F, quote = F)

source_data_4a <- cbind( "", "", res)
source_data_4a[1,1] <- "figure 4a"

write.csv(source_data_4a,"source_data/source_data_4a.csv", row.names = F)

# Microbiome_between_groups.csv has all the PERMANOVA results for all group-wise comparisions
