...
a <- lme4::lmer(map$Phenotype.score ~ map$discription + map$DPW + (1|map$Mouse.ID))
b <- lme4::lmer(map$Phenotype.score ~ map$DPW + (1|map$Mouse.ID))
...
a <- lme4::lmer(map$Phenotype.score ~ map$discription + map$Age.Days + (1|map$Mouse.ID))
b <- lme4::lmer(map$Phenotype.score ~ map$discription+ (1|map$Mouse.ID))
...
plot(map$Phenotype.score ~ map$DPW, pch = 21, bg = map$col, cex  =2, 
     ylab = "Phnotype Score", xlab = "DPW", font = 3, font.lab = 4)
