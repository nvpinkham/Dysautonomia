...
res2title <- function(res){
  res <- paste("BC dist ~ group p val = ", res$`Pr(>F)`[1],
               "\n R2 =", round(res$R2[1], 5), ", F stat =", round(res$F[1], 5),
               ", df =", res$Df[1])
  res
}

res2tab <- function(res){

  res1 <- c(res$`Pr(>F)`[1],round(res$R2[1], 5), round(res$F[1], 5),res$Df[1])

  names(res1) <- c("p val", "R2", "F stat", "df")
  res1
}
...
make_nmds_FDMS <- function(groups, return.points = T){
...
M2Rres <- make_nmds_FDMS(groups = c("Martinson", "Relative"), return.points = F)
M2Pres <- make_nmds_FDMS(groups = c("Martinson", "Patient"), return.points = F)
B2Rres <- make_nmds_FDMS(groups = c("Baxter", "Relative"), return.points = F)
B2Pres <- make_nmds_FDMS(groups = c("Baxter", "Patient"), return.points = F)
B2Mres <- make_nmds_FDMS(groups = c("Baxter", "Martinson"), return.points = F)
P2Rres <- make_nmds_FDMS(groups = c("Patient", "Relative"), return.points = F)


perm.res <- rbind(c("Martinson", "Relative", res2tab(M2Rres)),
                  c("Martinson", "Relative", res2tab(M2Pres)),
                  c("Baxter", "Relative", res2tab(B2Rres)),
                  c("Baxter", "Patient", res2tab(B2Pres)),
                  c("Baxter", "Martinson", res2tab(B2Mres)),
                  c("Patient", "Relative", res2tab(P2Rres)))

write.csv(perm.res, "Statistical_summaries/Sup_Fig_4d.csv", row.names = F)
...
make_nmds_FDMS(groups = c("Healthy", "Relative"))
otu.nmds <- make_nmds_FDMS(groups = c("Healthy", "Patient"))
