...
treat.res <- anova(phenotype.lme, Treatment.null) # significant
dpw.res <- anova(phenotype.lme, dpw.null) # significant


summary(phenotype.lme)
lme4::confint.merMod(phenotype.lme, level = .95)

treat.res
dpw.res

treat.sum <- c(treat.res$Df[2], treat.res$`Pr(>Chisq)`[2],  treat.res$Chisq[2],
               summary(phenotype.lme)$coefficients[2,1])
names(treat.sum) <- c("df", "p value", "Chi squared", "effect size")
...
#writeLines(capture.output(lme.res), "Statistical_summaries/Sup_6_tests.txt")

treat.sum <- list("map$Phenotype.score ~ map$Treatment.type + map$DPW + (1|map$Mouse.ID)",
                  "Treatment.type Result",
                  treat.sum,
                  "*treatment type = housing")


writeLines(capture.output(treat.sum), "Statistical_summaries/Sup_6_tests.txt")
