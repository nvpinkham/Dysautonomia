...
cor.perm <- function(var1 = map$eGFR,
  ...
  res <- as.data.frame(matrix(nrow =  num.perm, ncol = 3))
  colnames(res) <- c("p value", "n", "S")
  # S is the test statistic which is the sum of all squared rank differences
  ...
  for(i in 1 : num.perm){
    ...
    if(norm ){
      ...
      res$t[i] <- res.i$statistic
      res$cor[i] <- res.i$estimate
      res$n[i] <- length(var1[pick3])
      res$CI95.lo <- res.i$conf.int[1]
      res$CI95.hi <- res.i$conf.int[2]
      res$df <- res.i$parameter
...
# 1 AGE
cor.perm(map.pat$dist.cent, map.pat$age, map.pat$DonorID, 99)
## 2
cor.perm(map.pat$shared.proportion, map.pat$age, map.pat$DonorID, 99)
### 3
cor.perm(map.pat$dist2pair, map.pat$age, map.pat$DonorID, 99, norm = T)
...
shared.res
confint(shared.lme)
length(map.pat$shared.num[pick])
...
sum(table(map.pat$pam, map.pat$Oral.foods))

##########################

######## 8 ##########
table(map.pat$Antibiotic, map.pat$pam)
fisher.test(table(map.pat$Antibiotic, map.pat$pam * -1))
sum(table(map.pat$Antibiotic, map.pat$pam))
...
### 9 #################
cor.perm(var1 = map.pat$invsimp, var2 = map.pat$stool.choline, donor = map.pat$DonorID, num.perm = 99, norm = T)
#### 10 #####################
cor.perm(map.pat$richness, map.pat$stool.choline, map.pat$DonorID, 99, norm = T)

##### 11 #####################
cor.perm(map.pat$shared.num, map.pat$stool.choline, map.pat$DonorID, 99, norm = T)
...
cor.perm(log(map.pat$eGFR), map.pat$richness, map.pat$DonorID, 99, norm = T)

######## 15 ######################
cor.perm(log(map.pat$eGFR), map.pat$shared.num, map.pat$DonorID, 99, norm = T)

######### 16 ######################
cor.perm(log(map.pat$eGFR), map.pat$unique.num, map.pat$DonorID, 99, norm = T)
