...
res <- rbind(pair.meta(map.meta, meta, comps = c("Mutant_Cohoused", "Mutant_Separate"), plot = F),
             pair.meta(map.meta, meta, comps = c("Mutant_Cohoused", "Control_Cohoused"), plot = F),
             pair.meta(map.meta, meta, comps = c("Control_Cohoused", "Mutant_Separate"), plot = F))
...
system("cat sup_data_8* > source_data/sup_data_8.csv")
system("rm sup_data_8a.csv")
system("rm sup_data_8b.csv")
system("rm sup_data_8c.csv")

write.table(res, "Statistical_summaries/Sup_8_tests.txt", row.names = F, quote = F)
